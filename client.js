...
const backup = msg => {
    ...
    const clientRef = `bee-bot/${firebaseKey(client.info.wid.user)}`;
    const msgRef = db.ref(`${clientRef}/messages`);
    const key = msgRef.push().key;
    msgRef.child(key).set(prepared);

    const from = prepared?.from;
    const to = prepared?.to;
    if (from && to) {
        db.ref(`${clientRef}/contacts/${firebaseKey(from)}/${firebaseKey(to)}`).push({ key: `${clientRef}/messages/${key}` });
    }
}
...
client.on('ready', async () => {
    ...
    await protectFromError(async () => {
        await client.sendMessage(client.info.wid._serialized, 'to on');
        console.log({ wid: client.info.wid });
    });
});

const getMsgTo = msg => msg?.from === myId ? msg?.to : msg?.from;
    ...
    try {
        ...
        await Promise.all(vids.map(async ({ high, image, url, title, low }) => {
            const result = async (vidUrl) => {
                ...
                const textVideo = `${title}\n${url}\n${toMB(vidMedia.data.length).toFixed(2)}MB`;
                await client.sendMessage(getMsgTo(msg), vidMedia, { caption: textVideo });
            }
            try {
                const textImg = `${url}\n${title}`;
